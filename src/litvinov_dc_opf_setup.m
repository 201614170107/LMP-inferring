function om = Litvinov_dc_opf_setup(mpc, mpopt, l0, D, T, LF)
...
if dc
  loss = 0;
  Demand = sum(mpc.bus(:,PD))/baseMVA;
  l_loss = - LF' * mpc.bus(:,PD)/baseMVA + l0/baseMVA;
  load = mpc.bus(:,PD)/baseMVA;
  Tmax = mpc.branch(:,RATE_A)/baseMVA + T*load;
  Tmin = -mpc.branch(:,RATE_A)/baseMVA + T*load;
  ...
  om = add_vars(om, 'loss', 1, loss, -1e5, 1e5);
  om = add_constraints(om, 'Pmis', [ones(1,ng),-1], Demand, Demand, {'Pg', 'loss'}); %% nb
  om = add_constraints(om, 'Ploss', [-LF(GEN_IDX,:)', 1], l_loss, l_loss, {'Pg', 'loss'});     %% nl2
  om = add_constraints(om, 'Pf', [T(:,GEN_IDX), -T(:,GEN_IDX)*D(GEN_IDX,:)], Tmin, Tmax, {'Pg', 'loss'});
